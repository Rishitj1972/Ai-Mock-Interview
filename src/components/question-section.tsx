...
export const QuestionSection = ({ questions }: QuestionSectionProps) => {
    ...
    const handlePlayQuestion = (qst : string) => {
        if(isPlaying && currentSpeech) {
            ...
            if("speechSynthesis" in window) {
                const speech = new SpeechSynthesisUtterance(qst); // create a new speech synthesis utterance
                window.speechSynthesis.speak(speech); // speak the utterance
  ...
  return (
    <div className="w-full min-h-96 border rounded-md p-4">
        ...
        >
            <TabsList
            className="bg-transparent w-full flex flex-wrap items-center justify-start gap-4">
            {questions?.map((tab,i) => (
                <TabsTrigger
                className={cn(
                    "data-[state=active]:bg-indigo-200 data-[state=active]:shadow-md text-xs px-2"
                )}
                key={tab.question}
                value={tab.question}
                >
